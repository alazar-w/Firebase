...
import com.example.tabianconsulting.utility.FilePaths;
...
//        }
        ...
        mSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!mName.getText().toString().equals("")){
                    //this is resulting false i need to know why??????????
                    Log.d(TAG, "onClick: CHECK"  + mName.getText().toString().trim().equals(mCurrentName.trim()));
                    if (!mName.getText().toString().equals(mCurrentName)){
                        ...
                        Toast.makeText(SettingsActivity.this,"No Name Change Made",Toast.LENGTH_SHORT).show();
                    }

                }
                //change phone
                if (!mPhone.getText().toString().equals("")){
                    if (!mPhone.getText().toString().equals(mCurrentPhone)){
                        reference.child(getString(R.string.dbnode_users))
                                .child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                .child(getString(R.string.field_phone))
                                .setValue(mPhone.getText().toString());
                        Toast.makeText(SettingsActivity.this,"Phone Updated",Toast.LENGTH_LONG).show();
                    }else {
                        Toast.makeText(SettingsActivity.this,"No Phone Change Made",Toast.LENGTH_SHORT).show();
                    }
    ...
    private void setCurrentUserProfileData(){
       ...
       */
       ...
       //with the orderByKey method it will search for the key encapsulating the values of the object
            ...
            query1.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    for (DataSnapshot singleSnapshot : snapshot.getChildren()){
                        //we use the getValue method and we specify what the data structure is going to look like
                        //by referencing the class(in this case the "User class")
                        User user1 = singleSnapshot.getValue(User.class);
                        Log.d(TAG, "onDataChange: QUERY METHOD 1 found user:" + user1.toString());
                        mPhone.setText(user1.getName());
                        mName.setText(user1.getPhone());
                        mCurrentName = user1.getName();
                        ImageLoader.getInstance().displayImage(user1.getProfile_image(),mProfileImage);
                        Log.d(TAG, "onDataChange: CURRENT NAME:" + user1.getName());
                        Log.d(TAG, "onDataChange: CURRENT PHONE:" + user1.getPhone());
                        mCurrentPhone = user1.getPhone();
                    }
                }
            ...
            --------- Query method 2 ----------
         */
            //with the orderByChild method it will look for the name of the field,which would be here "user_id"(orderByChild looks for field name)
...
//                        User user2 = singleSnapshot.getValue(User.class);
//                        Log.d(TAG, "onDataChange: (QUERY METHOD 2) found user: " + user2.toString());
//
